...
function help_exit
    ...
    echo "-d/--divide: Divide in pair-ends"
...
set options $options (fish_opt -s d -l divide)
...
set -q _flag_s; or set -l _flag_s ~/ratsvm/SAMtools
...
for fname in $fnames
	...
	if set -q _flag_d
	    ...
	    echo "samtools view -f 13 $fname _$argv.sam | samtools sort - -n -O sam -@ $_flag_p | samtools fastq - > $fname _unmap.fastq"
	    samtools view -f 13 "$fname"_"$argv".sam | samtools sort - -n -O sam -@ "$_flag_p" | samtools fastq - > "$fname"_unmap.fastq 
	end
	echo "<<< "
end
